...
## 화면 설정
title = "🏆 전국 전기차 보조금 비교"
st.set_page_config(page_title="전국 전기차 보조금 비교",page_icon="🏆",layout="wide",)
st.title(title)
st.markdown(
...
try:
    ...
    st.markdown("🌟login 팝업 창에서 MySQL username과 password를 입력해주세요🌟")
    st.markdown("""만일 SKN06-1st-6Team 폴더에서 init.py 파일을 실행시키지 않았다면 다음 과정을 따라해주세요.
1. requirements.txt 파일을 통해 패키지를 다운받습니다.
```
pip install -r requirements.txt
```
2. init.py 파일을 실행시킵니다.
```
python server/init.py
```
3. app.py를 streamlit을 통해 실행시킵니다. 또는 현재 화면에서 R을 입력합니다.
```
